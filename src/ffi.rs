...
unsafe fn c_to_pattern(pattern: *const FcPatternC) -> FcPattern {
    ...
    if !pattern.unicode_ranges.is_null() && pattern.unicode_ranges_count > 0 {
        unicode_ranges = slice::from_raw_parts(pattern.unicode_ranges, pattern.unicode_ranges_count)
            .to_vec();
    }
...
unsafe fn free_font_match_c(match_obj: *mut FcFontMatchC) {
    ...
    if !match_obj.fallbacks.is_null() && match_obj.fallbacks_count > 0 {
        let fallbacks = slice::from_raw_parts_mut(
            match_obj.fallbacks,
...
fn trace_msgs_to_c(trace: &[TraceMsg]) -> (*mut FcTraceMsgC, usize) {
    ...
    for msg in trace {
        let path = CString::new(msg.path.clone()).unwrap_or_default().into_raw();
...
pub extern "C" fn fc_pattern_add_unicode_range(
    ...
    unsafe {
        ...
        if !pattern.unicode_ranges.is_null() && pattern.unicode_ranges_count > 0 {
            new_ranges.extend_from_slice(
                slice::from_raw_parts(pattern.unicode_ranges, pattern.unicode_ranges_count)
            );
