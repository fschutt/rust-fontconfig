...
use rust_fontconfig::{
    FcFallbackConfig, FcFont, FcFontCache, FcPattern, FcScriptFallback, FcWeight,
    FontFallbackChain, FontId, GenericFamily, OperatingSystem, PatternMatch, UnicodeRange,
...
fn windows_like_cache(config: FcFallbackConfig) -> FcFontCache {
    ...
    cache.with_memory_fonts(vec![
        mock(
            "Segoe UI",
            &[BASIC_LATIN, LATIN_1, GREEK, CYRILLIC, HEBREW, ARABIC],
        ),
        mock(
            "MS Gothic",
            &[
...
fn sans_serif_with_windows_defaults_prefers_ms_gothic_over_unifont_for_japanese() {
    ...
    assert_eq!(
        resolved(&cache, &chain, 'A'),
        ("Segoe UI".into(), "sans-serif".into())
    );
    assert_eq!(
        resolved(&cache, &chain, 'Я'),
        ("Segoe UI".into(), "sans-serif".into())
    );
    assert_eq!(
        resolved(&cache, &chain, 'ب'),
        ("Segoe UI".into(), "sans-serif".into())
    );
    assert_eq!(
        resolved(&cache, &chain, 'あ'),
        ("MS Gothic".into(), "sans-serif".into()),
        "{chain:#?}"
    );
    assert_eq!(
        resolved(&cache, &chain, '漢'),
        ("MS Gothic".into(), "sans-serif".into()),
        "{chain:#?}"
    );
    ...
    assert!(
        chain.unicode_fallbacks.iter().all(|g| g
            .fonts
            .iter()
            .all(|m| family_of(&cache, m.id) != "MS Gothic")),
...
fn without_any_configuration_the_dedicated_font_beats_the_pan_unicode_font() {
    ...
    let order: Vec<String> = hiragana
        .fonts
        .iter()
        .map(|m| family_of(&cache, m.id))
        .collect();
...
fn unconfigured_generic_resolves_narrow_before_wide() {
    ...
    let base: Vec<String> = chain.css_fallbacks[0]
        .fonts
        .iter()
        .map(|m| family_of(&cache, m.id))
        .collect();
...
fn configured_script_preference_beats_the_ranking() {
    ...
    config
        .generic_families
        .insert(GenericFamily::SansSerif, vec!["Segoe UI".to_string()]);
...
fn stack_without_a_generic_uses_the_default_generic_preferences() {
    ...
    assert_eq!(
        resolved(&cache, &chain, 'あ'),
        ("Unifont".into(), "(unicode-fallback)".into()),
        "{chain:#?}"
    );
}
...
fn last_resort_is_used_without_a_coverage_check() {
    ...
    assert_eq!(
        resolved(&cache, &chain, 'あ'),
        ("Segoe UI".into(), "(last-resort)".into())
    );
    assert_eq!(
        resolved(&cache, &chain, 'A'),
        ("Segoe UI".into(), "Segoe UI".into())
...
fn substitutions_stand_in_for_missing_named_families() {
    ...
    config
        .substitutions
        .insert("arial".to_string(), vec!["Segoe UI".to_string()]);
    let cache = windows_like_cache(config);
    let chain = chain(&cache, &["Arial"], Some(&[]));

    assert_eq!(
        resolved(&cache, &chain, 'A'),
        ("Segoe UI".into(), "Arial".into()),
        "{chain:#?}"
    );
}
...
fn registry_prefetch_list_covers_the_chain() {
    ...
    let families = config.candidate_families(
    ...
    for m in chain.fonts() {
        ...
        assert!(
            families.iter().any(|f| f.eq_ignore_ascii_case(&name)),
            "{name} resolved but was not prefetched"
        );
    }
}
